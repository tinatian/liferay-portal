...
public class KBCommentPersistenceImpl
	...
	public void activate(BundleContext bundleContext) {
		...
		_finderPathWithPaginationCountByC_C_S = _createFinderPath(
			...
			new String[] {"classNameId", "classPK", "status"});
	}
	...
	private static class KBCommentModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(kbCommentModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			KBCommentModelImpl kbCommentModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] = kbCommentModelImpl.getOriginalAttributeValue(
						attributeName);
				}
				else {
					arguments[i] = kbCommentModelImpl.getAttributeValue(
						attributeName);
				}
			}
