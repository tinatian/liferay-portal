	...
	public void activate(BundleContext bundleContext) {
		...
		_argumentsResolverServiceRegistration = _bundleContext.registerService(
			...
			MapUtil.singletonDictionary(
				"model.class.name", DDLRecordSetVersion.class.getName()));
		...
		_finderPathCountByRS_S = _createFinderPath(
			Long.class, FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION,
			"countByRS_S", new String[] {"recordSetId", "status"});
	}
	...
	private static class DDLRecordSetVersionModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					ddlRecordSetVersionModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						ddlRecordSetVersionModelImpl.getOriginalAttributeValue(
							attributeName);
				}
				else {
					arguments[i] =
						ddlRecordSetVersionModelImpl.getAttributeValue(
							attributeName);
				}
			}
