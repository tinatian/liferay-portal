	...
	private static class OrgGroupRoleModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					orgGroupRoleModelImpl, attributeNames, original);
			}
			...
			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |=
					orgGroupRoleModelImpl.getColumnBitmask(attributeName);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						orgGroupRoleModelImpl.getOriginalAttributeValue(
							attributeName);
				}
				else {
					arguments[i] = orgGroupRoleModelImpl.getAttributeValue(
						attributeName);
				}
			}
