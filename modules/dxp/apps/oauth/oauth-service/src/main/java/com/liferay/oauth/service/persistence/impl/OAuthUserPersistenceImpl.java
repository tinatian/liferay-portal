...
public class OAuthUserPersistenceImpl
	...
	public void activate(BundleContext bundleContext) {
		...
		_finderPathWithPaginationFindByOAuthApplicationId = _createFinderPath(
			OAuthUserImpl.class, FINDER_CLASS_NAME_LIST_WITH_PAGINATION,
			"findByOAuthApplicationId", new String[] {"oAuthApplicationId"});

		_finderPathWithoutPaginationFindByOAuthApplicationId =
			_createFinderPath(
				OAuthUserImpl.class, FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION,
				"findByOAuthApplicationId",
				new String[] {"oAuthApplicationId"});

		_finderPathCountByOAuthApplicationId = _createFinderPath(
			Long.class, FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION,
			"countByOAuthApplicationId", new String[] {"oAuthApplicationId"});
	...
	private static class OAuthUserModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(oAuthUserModelImpl, attributeNames, original);
			}

			long finderPathColumnBitmask = 0;

			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |= oAuthUserModelImpl.getColumnBitmask(
					attributeName);
			}
		...
		private Object[] _getValue(
			OAuthUserModelImpl oAuthUserModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] = oAuthUserModelImpl.getOriginalAttributeValue(
						attributeName);
				}
				else {
					arguments[i] = oAuthUserModelImpl.getAttributeValue(
						attributeName);
				}
			}
