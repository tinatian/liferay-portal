	...
	private static class CountryModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(countryModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			CountryModelImpl countryModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] = countryModelImpl.getOriginalAttributeValue(
						attributeName);
				}
				else {
					arguments[i] = countryModelImpl.getAttributeValue(
						attributeName);
				}
			}
