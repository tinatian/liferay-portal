...
/**
 * The arguments resolver class for retrieving value from CommerceCatalog.
 *
 * @author Marco Leo
 * @generated
 */
@Component(
	immediate = true,
	service = {
		CommerceCatalogModelArgumentsResolver.class, ArgumentsResolver.class
	}
)
public class CommerceCatalogModelArgumentsResolver
	...
	public Object[] getArguments(
		...
		if (!checkColumn || (columnBitmask == 0)) {
			return _getValue(commerceCatalogModelImpl, columnNames, original);
		}
		...
		if (finderPathColumnBitmask == null) {
			...
			if (finderPath.isBaseModelResult() &&
				(CommerceCatalogPersistenceImpl.
	...
	private static Object[] _getValue(
		CommerceCatalogModelImpl commerceCatalogModelImpl, String[] columnNames,
		boolean original) {
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			if (original) {
				arguments[i] = commerceCatalogModelImpl.getColumnOriginalValue(
					columnName);
			}
			else {
	...
	static {
		...
		orderByColumnsBitmask |= CommerceCatalogModelImpl.getColumnBitmask(
			"createDate");
