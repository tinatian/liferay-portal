	...
	private static class SourceModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(sourceModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			SourceModelImpl sourceModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] = sourceModelImpl.getOriginalAttributeValue(
						attributeName);
				}
				else {
					arguments[i] = sourceModelImpl.getAttributeValue(
						attributeName);
				}
			}
