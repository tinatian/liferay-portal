...
public class OAuthApplicationPersistenceImpl
	...
	public void activate(BundleContext bundleContext) {
		...
		_finderPathFetchByConsumerKey = _createFinderPath(
			...
			"fetchByConsumerKey", new String[] {"consumerKey"});

		_finderPathCountByConsumerKey = _createFinderPath(
			Long.class, FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION,
			"countByConsumerKey", new String[] {"consumerKey"});

		_finderPathWithPaginationFindByC_N = _createFinderPath(
			OAuthApplicationImpl.class, FINDER_CLASS_NAME_LIST_WITH_PAGINATION,
			"findByC_N", new String[] {"companyId", "name"});
		...
		_finderPathWithPaginationFindByU_N = _createFinderPath(
			OAuthApplicationImpl.class, FINDER_CLASS_NAME_LIST_WITH_PAGINATION,
			"findByU_N", new String[] {"userId", "name"});
	...
	private static class OAuthApplicationModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					oAuthApplicationModelImpl, attributeNames, original);
			}
			...
			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |=
					oAuthApplicationModelImpl.getColumnBitmask(attributeName);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						oAuthApplicationModelImpl.getOriginalAttributeValue(
							attributeName);
				}
				else {
					arguments[i] = oAuthApplicationModelImpl.getAttributeValue(
						attributeName);
				}
			}
