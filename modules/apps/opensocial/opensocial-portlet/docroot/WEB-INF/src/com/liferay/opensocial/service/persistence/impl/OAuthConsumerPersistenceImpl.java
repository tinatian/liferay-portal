...
public class OAuthConsumerPersistenceImpl extends BasePersistenceImpl<OAuthConsumer>
	...
	private static final String _FINDER_COLUMN_GADGETKEY_GADGETKEY_2_SQL = "oAuthConsumer.gadgetKey = ?";
	private static final String _FINDER_COLUMN_GADGETKEY_GADGETKEY_3_SQL = "(oAuthConsumer.gadgetKey IS NULL OR oAuthConsumer.gadgetKey = '')";
	...
	public OAuthConsumer fetchByG_S(String gadgetKey, String serviceName,
		...
		if (retrieveFromCache) {
			result = FinderCacheUtil.getResult(_finderPathFetchByG_S,
					finderArgs, this);
		}
		...
		if (result == null) {
			...
			try {
				...
				if (list.isEmpty()) {
					FinderCacheUtil.putResult(_finderPathFetchByG_S,
						finderArgs, list);
				}
				else {
					if (list.size() > 1) {
	...
	private static final String _FINDER_COLUMN_G_S_GADGETKEY_2_SQL = "oAuthConsumer.gadgetKey = ? AND ";
	private static final String _FINDER_COLUMN_G_S_GADGETKEY_3_SQL = "(oAuthConsumer.gadgetKey IS NULL OR oAuthConsumer.gadgetKey = '') AND ";
	private static final String _FINDER_COLUMN_G_S_SERVICENAME_2_SQL = "oAuthConsumer.serviceName = ?";
	private static final String _FINDER_COLUMN_G_S_SERVICENAME_3_SQL = "(oAuthConsumer.serviceName IS NULL OR oAuthConsumer.serviceName = '')";
	...
	protected void cacheUniqueFindersCache(
		...
		FinderCacheUtil.putResult(_finderPathCountByG_S, args, Long.valueOf(1),
			false);
		FinderCacheUtil.putResult(_finderPathFetchByG_S, args,
	...
	public OAuthConsumer updateImpl(OAuthConsumer oAuthConsumer) {
		...
		if (!OAuthConsumerModelImpl.COLUMN_BITMASK_ENABLED) {
		 ...
		 if (isNew) {
			...
			FinderCacheUtil.removeResult(_finderPathWithoutPaginationFindByGadgetKey,
				args);
			...
			if ((oAuthConsumerModelImpl.getColumnBitmask() &
					_finderPathWithoutPaginationFindByGadgetKey.getColumnBitmask()) != 0) {
				Object[] args = new Object[] {
						oAuthConsumerModelImpl.getOriginalGadgetKey()
					};
