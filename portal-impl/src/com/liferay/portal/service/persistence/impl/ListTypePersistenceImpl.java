	...
	private static class ListTypeModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(listTypeModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			ListTypeModelImpl listTypeModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] = listTypeModelImpl.getOriginalAttributeValue(
						attributeName);
				}
				else {
					arguments[i] = listTypeModelImpl.getAttributeValue(
						attributeName);
				}
			}
