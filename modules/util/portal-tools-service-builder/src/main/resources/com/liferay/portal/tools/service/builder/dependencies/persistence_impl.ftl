...
public class ${entity.name}PersistenceImpl extends BasePersistenceImpl<${entity.name}> implements ${entity.name}Persistence {
	...
	public ${entity.name} updateImpl(${apiPackagePath}.model.${entity.name} ${entity.varName}) {
		...
		<#if serviceBuilder.isVersionGTE_7_3_0()>
			${entity.name} original${entity.name} = ${entity.varName};
		</#if>
		...
		<#if serviceBuilder.isVersionGTE_7_3_0()>
			${entityCache}.putResult(${entity.name}Impl.class, original${entity.name}, false, true);
	...
	<#else>
		...
		_finderPathWithPaginationFindAll =
			<#if serviceBuilder.isVersionGTE_7_3_0()>
				_createFinderPath(
					FINDER_CLASS_NAME_LIST_WITH_PAGINATION, "all",
			<#else>
				new FinderPath(
					${entityCacheEnabled},
					${finderCacheEnabled},
		...
		_finderPathWithoutPaginationFindAll =
			<#if serviceBuilder.isVersionGTE_7_3_0()>
				_createFinderPath(
				FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION, "all",
			<#else>
				new FinderPath(
					${entityCacheEnabled},
					${finderCacheEnabled},
		...
		<#list entity.entityFinders as entityFinder>
			...
			<#if entityFinder.isCollection()>
				_finderPathWithPaginationFindBy${entityFinder.name} =
					<#if serviceBuilder.isVersionGTE_7_3_0()>
						_createFinderPath(
							FINDER_CLASS_NAME_LIST_WITH_PAGINATION, "${entityFinder.name}",
					<#else>
						new FinderPath(
							${entityCacheEnabled},
							${finderCacheEnabled},
				...
				<#if !entityFinder.hasCustomComparator()>
					_finderPathWithoutPaginationFindBy${entityFinder.name} =
						<#if serviceBuilder.isVersionGTE_7_3_0()>
							_createFinderPath(
								FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION, "${entityFinder.name}",
						<#else>
							new FinderPath(
								${entityCacheEnabled},
								${finderCacheEnabled},
			...
			<#if !entityFinder.isCollection() || entityFinder.isUnique()>
				_finderPathFetchBy${entityFinder.name} =
					<#if serviceBuilder.isVersionGTE_7_3_0()>
						_createFinderPath(
							FINDER_CLASS_NAME_ENTITY, "${entityFinder.name}",
					<#else>
						new FinderPath(
							${entityCacheEnabled},
							${finderCacheEnabled},
