...
public class UserTrackerPathPersistenceImpl
	...
	public void afterPropertiesSet() {
		...
		_finderPathWithPaginationFindByUserTrackerId = _createFinderPath(
			UserTrackerPathImpl.class, FINDER_CLASS_NAME_LIST_WITH_PAGINATION,
			"findByUserTrackerId", new String[] {"userTrackerId"});
		...
		_finderPathWithoutPaginationFindByUserTrackerId = _createFinderPath(
			...
			new String[] {"userTrackerId"});

		_finderPathCountByUserTrackerId = _createFinderPath(
			Long.class, FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION,
			"countByUserTrackerId", new String[] {"userTrackerId"});
	}
	...
	private static class UserTrackerPathModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					userTrackerPathModelImpl, attributeNames, original);
			}
			...
			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |=
					userTrackerPathModelImpl.getColumnBitmask(attributeName);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						userTrackerPathModelImpl.getOriginalAttributeValue(
							attributeName);
				}
				else {
					arguments[i] = userTrackerPathModelImpl.getAttributeValue(
						attributeName);
				}
			}
