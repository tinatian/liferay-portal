...
public class WeDeployAuthTokenPersistenceImpl
	...
	public void activate(BundleContext bundleContext) {
		...
		_finderPathFetchByCI_T_T = _createFinderPath(
			...
			"fetchByCI_T_T", new String[] {"clientId", "token", "type"});

		_finderPathCountByCI_T_T = _createFinderPath(
			Long.class, FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION,
			"countByCI_T_T", new String[] {"clientId", "token", "type"});
	}
	...
	private static class WeDeployAuthTokenModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					weDeployAuthTokenModelImpl, attributeNames, original);
			}
			...
			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |=
					weDeployAuthTokenModelImpl.getColumnBitmask(attributeName);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					...
					arguments[i] = weDeployAuthTokenModelImpl.getAttributeValue(
						attributeName);
				}
			}
