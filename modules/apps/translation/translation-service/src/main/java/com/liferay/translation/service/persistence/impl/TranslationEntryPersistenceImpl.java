	...
	public void activate(BundleContext bundleContext) {
		...
		_argumentsResolverServiceRegistration = _bundleContext.registerService(
			...
			MapUtil.singletonDictionary(
				"model.class.name", TranslationEntry.class.getName()));
		...
		_finderPathFetchByC_C_L = _createFinderPath(
			TranslationEntryImpl.class, FINDER_CLASS_NAME_ENTITY,
			"fetchByC_C_L",
			new String[] {"classNameId", "classPK", "languageId"});

		_finderPathCountByC_C_L = _createFinderPath(
			Long.class, FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION,
			"countByC_C_L",
			new String[] {"classNameId", "classPK", "languageId"});
	}
	...
	private static class TranslationEntryModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					translationEntryModelImpl, attributeNames, original);
			}
			...
			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |=
					translationEntryModelImpl.getColumnBitmask(attributeName);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					...
					arguments[i] = translationEntryModelImpl.getAttributeValue(
						attributeName);
				}
			}
