...
public class KaleoProcessPersistenceImpl
	...
	public void cacheResult(KaleoProcess kaleoProcess) {
		entityCache.putResult(
			entityCacheEnabled, KaleoProcessImpl.class,
	...
	public void cacheResult(List<KaleoProcess> kaleoProcesses) {
		for (KaleoProcess kaleoProcess : kaleoProcesses) {
			if (entityCache.getResult(
					entityCacheEnabled, KaleoProcessImpl.class,
					kaleoProcess.getPrimaryKey()) == null) {
	...
	public void clearCache(KaleoProcess kaleoProcess) {
		...
		entityCache.removeResult(
			entityCacheEnabled, KaleoProcessImpl.class, kaleoProcess,
	...
	public void clearCache(Set<Serializable> primaryKeys) {
		...
		for (Serializable primaryKey : primaryKeys) {
			entityCache.removeResult(
				entityCacheEnabled, KaleoProcessImpl.class, primaryKey);
		}
	}
	...
	public KaleoProcess updateImpl(KaleoProcess kaleoProcess) {
		...
		entityCache.putResult(
			entityCacheEnabled, KaleoProcessImpl.class, kaleoProcess, false,
			_columnBitmaskEnabled);
	...
	public void activate(BundleContext bundleContext) {
		KaleoProcessModelImpl.setEntityCacheEnabled(entityCacheEnabled);
		KaleoProcessModelImpl.setFinderCacheEnabled(finderCacheEnabled);

		_finderPathWithPaginationFindAll = new FinderPath(
			finderCacheEnabled, KaleoProcessImpl.class,
		...
		_finderPathFetchByDDLRecordSetId = new FinderPath(
			...
			new String[] {Long.class.getName()},
			_ARGUMENTS_BIFUNCTION_MAP.get("DDLRecordSetId"),
			_ORIGINAL_ARGUMENTS_BIFUNCTION_MAP.get("DDLRecordSetId"));

		_serviceRegistrations.add(
			bundleContext.registerService(
				FinderPath.class, _finderPathFetchByDDLRecordSetId,
				MapUtil.singletonDictionary(
					"cache.name", FINDER_CLASS_NAME_ENTITY)));
		...
		_serviceRegistrations.add(
			bundleContext.registerService(
				FinderPath.class, _finderPathCountByDDLRecordSetId,
				MapUtil.singletonDictionary(
					"cache.name", FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION)));
	}
	...
	static {
		_FINDER_PATH_BITMASK_MAP.put(
			"Uuid", KaleoProcessModelImpl.UUID_COLUMN_BITMASK);
		...
		_FINDER_PATH_BITMASK_MAP.put(
			"GroupId", KaleoProcessModelImpl.GROUPID_COLUMN_BITMASK);
	...
	static {
		_ARGUMENTS_BIFUNCTION_MAP.put(
			...
			(baseModel, checkColumns) -> {
				...
				if (!checkColumns ||
					((kaleoProcessModelImpl.getColumnBitmask() &
					  _FINDER_PATH_BITMASK_MAP.get("Uuid")) != 0)) {

					return new Object[] {kaleoProcessModelImpl.getUuid()};
				}
		...
		_ARGUMENTS_BIFUNCTION_MAP.put(
			...
			(baseModel, checkColumns) -> {
				...
				if (!checkColumns ||
					...
					return new Object[] {kaleoProcessModelImpl.getGroupId()};
				}
		...
		_ARGUMENTS_BIFUNCTION_MAP.put(
			...
			(baseModel, checkColumns) -> {
				...
				if (!checkColumns ||
					((kaleoProcessModelImpl.getColumnBitmask() &
					  _FINDER_PATH_BITMASK_MAP.get("DDLRecordSetId")) != 0)) {
