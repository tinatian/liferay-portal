...
public class KBFolderPersistenceImpl
	...
	public void clearCache(KBFolder kbFolder) {
		...
		entityCache.removeResult(
			entityCacheEnabled, KBFolderImpl.class, kbFolder,
	...
	public KBFolder updateImpl(KBFolder kbFolder) {
		...
		entityCache.putResult(
			entityCacheEnabled, KBFolderImpl.class, kbFolder, false,
			_columnBitmaskEnabled);
	...
	public void activate(BundleContext bundleContext) {
		...
		_finderPathWithPaginationFindAll = new FinderPath(
			finderCacheEnabled, KBFolderImpl.class,
		...
		_finderPathFetchByUUID_G = new FinderPath(
			finderCacheEnabled, KBFolderImpl.class, FINDER_CLASS_NAME_ENTITY,
			"fetchByUUID_G",
		...
		_finderPathFetchByG_P_UT = new FinderPath(
			...
			_ARGUMENTS_BIFUNCTION_MAP.get("G_P_UT"),
			_ORIGINAL_ARGUMENTS_BIFUNCTION_MAP.get("G_P_UT"));

		_serviceRegistrations.add(
			bundleContext.registerService(
				FinderPath.class, _finderPathFetchByG_P_UT,
				MapUtil.singletonDictionary(
					"cache.name", FINDER_CLASS_NAME_ENTITY)));
		...
		_serviceRegistrations.add(
			bundleContext.registerService(
				FinderPath.class, _finderPathCountByG_P_UT,
				MapUtil.singletonDictionary(
					"cache.name", FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION)));
	}
	...
	static {
		_FINDER_PATH_BITMASK_MAP.put(
			"Uuid", KBFolderModelImpl.UUID_COLUMN_BITMASK);
	...
	static {
		_ARGUMENTS_BIFUNCTION_MAP.put(
			...
			(baseModel, checkColumns) -> {
				KBFolderModelImpl kbFolderModelImpl =
					(KBFolderModelImpl)baseModel;

				if (!checkColumns ||
					((kbFolderModelImpl.getColumnBitmask() &
					  _FINDER_PATH_BITMASK_MAP.get("Uuid")) != 0)) {

					return new Object[] {kbFolderModelImpl.getUuid()};
				}
		...
		_ORIGINAL_ARGUMENTS_BIFUNCTION_MAP.put(
			...
			(baseModel, checkColumns) -> {
				...
				if (!checkColumns ||
					...
					return new Object[] {kbFolderModelImpl.getOriginalUuid()};
				}
		...
		_ARGUMENTS_BIFUNCTION_MAP.put(
			...
			(baseModel, checkColumns) -> {
				...
				if (!checkColumns ||
					((kbFolderModelImpl.getColumnBitmask() &
					  _FINDER_PATH_BITMASK_MAP.get("G_P_UT")) != 0)) {
