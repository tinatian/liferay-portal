	...
	public void activate(BundleContext bundleContext) {
		...
		_argumentsResolverServiceRegistration = _bundleContext.registerService(
			...
			MapUtil.singletonDictionary(
				"model.class.name", SamlSpIdpConnection.class.getName()));
		...
		_finderPathFetchByC_SIEI = _createFinderPath(
			...
			"fetchByC_SIEI", new String[] {"companyId", "samlIdpEntityId"});

		_finderPathCountByC_SIEI = _createFinderPath(
			Long.class, FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION,
			"countByC_SIEI", new String[] {"companyId", "samlIdpEntityId"});
	}
	...
	private static class SamlSpIdpConnectionModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					samlSpIdpConnectionModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					...
					arguments[i] =
						samlSpIdpConnectionModelImpl.getAttributeValue(
							attributeName);
				}
			}
