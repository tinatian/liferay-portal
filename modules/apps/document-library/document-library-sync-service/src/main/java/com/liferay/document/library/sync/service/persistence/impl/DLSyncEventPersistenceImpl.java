...
public class DLSyncEventPersistenceImpl
	...
	public void activate(BundleContext bundleContext) {
		...
		_finderPathWithPaginationFindByModifiedTime = _createFinderPath(
			DLSyncEventImpl.class, FINDER_CLASS_NAME_LIST_WITH_PAGINATION,
			"findByModifiedTime", new String[] {"modifiedTime"});

		_finderPathWithPaginationCountByModifiedTime = _createFinderPath(
			Long.class, FINDER_CLASS_NAME_LIST_WITH_PAGINATION,
			"countByModifiedTime", new String[] {"modifiedTime"});
		...
		_finderPathCountByTypePK = _createFinderPath(
			...
			"countByTypePK", new String[] {"typePK"});
	}
	...
	private static class DLSyncEventModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					dlSyncEventModelImpl, attributeNames, original);
			}
			...
			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |=
					dlSyncEventModelImpl.getColumnBitmask(attributeName);
			}
		...
		private Object[] _getValue(
			DLSyncEventModelImpl dlSyncEventModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						dlSyncEventModelImpl.getOriginalAttributeValue(
							attributeName);
				}
				else {
					arguments[i] = dlSyncEventModelImpl.getAttributeValue(
						attributeName);
				}
			}
