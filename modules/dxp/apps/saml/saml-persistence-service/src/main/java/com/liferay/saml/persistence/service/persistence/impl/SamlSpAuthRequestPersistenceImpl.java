...
public class SamlSpAuthRequestPersistenceImpl
	...
	public void activate(BundleContext bundleContext) {
		...
		_finderPathFetchBySIEI_SSARK = _createFinderPath(
			...
			"fetchBySIEI_SSARK",
			new String[] {"samlIdpEntityId", "samlSpAuthRequestKey"});
	...
	private static class SamlSpAuthRequestModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					samlSpAuthRequestModelImpl, attributeNames, original);
			}
			...
			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |=
					samlSpAuthRequestModelImpl.getColumnBitmask(attributeName);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					...
					arguments[i] = samlSpAuthRequestModelImpl.getAttributeValue(
						attributeName);
				}
			}
