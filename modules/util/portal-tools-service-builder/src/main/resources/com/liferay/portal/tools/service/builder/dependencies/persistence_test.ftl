				...
				<#list entityColumns as entityColumn>
					<#if entityColumn.isInterfaceColumn()>
						<#if stringUtil.equals(entityColumn.type, "double")>
							AssertUtils.assertEquals(existing${entity.name}.get${entityColumn.methodName}(), ReflectionTestUtil.<Double>invoke(existing${entity.name}, "getCacheModelAttribute", new Class<?>[]{String.class}, ${entityColumn.name}));
						<#elseif entityColumn.isPrimitiveType()>
							Assert.assertEquals(${serviceBuilder.getPrimitiveObj(entityColumn.type)}.valueOf(existing${entity.name}.get${entityColumn.methodName}()), ReflectionTestUtil.<${serviceBuilder.getPrimitiveObj(entityColumn.type)}>invoke(existing${entity.name}, "getCacheModelAttribute", new Class<?>[]{String.class}, ${entityColumn.name}));
						<#else>
							Assert.assertTrue(Objects.equals(existing${entity.name}.get${entityColumn.methodName}(), ReflectionTestUtil.invoke(existing${entity.name}, "getCacheModelAttribute", new Class<?>[]{String.class}, ${entityColumn.name})));
						</#if>
					</#if>
				</#list>
			</#list>
