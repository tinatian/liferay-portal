...
public class RedirectEntryPersistenceImpl
	...
	public void activate(BundleContext bundleContext) {
		...
		_finderPathWithPaginationFindByG_D = _createFinderPath(
			RedirectEntryImpl.class, FINDER_CLASS_NAME_LIST_WITH_PAGINATION,
			"findByG_D", new String[] {"groupId", "destinationURL"});
	...
	private static class RedirectEntryModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					redirectEntryModelImpl, attributeNames, original);
			}
			...
			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |=
					redirectEntryModelImpl.getColumnBitmask(attributeName);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					...
					arguments[i] = redirectEntryModelImpl.getAttributeValue(
						attributeName);
				}
			}
