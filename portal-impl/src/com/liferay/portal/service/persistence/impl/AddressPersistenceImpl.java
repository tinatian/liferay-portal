...
public class AddressPersistenceImpl
	...
	public void afterPropertiesSet() {
		...
		_finderPathWithPaginationFindByC_C_C_P = _createFinderPath(
			AddressImpl.class, FINDER_CLASS_NAME_LIST_WITH_PAGINATION,
			"findByC_C_C_P",
			new String[] {"companyId", "classNameId", "classPK", "primary"});
	...
	private static class AddressModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(addressModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			AddressModelImpl addressModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] = addressModelImpl.getOriginalAttributeValue(
						attributeName);
				}
				else {
					arguments[i] = addressModelImpl.getAttributeValue(
						attributeName);
				}
			}
