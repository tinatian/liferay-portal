...
public class ExpandoTablePersistenceImpl
	...
	public void afterPropertiesSet() {
		...
		_finderPathCountByC_C_N = _createFinderPath(
			...
			"countByC_C_N", new String[] {"companyId", "classNameId", "name"});
	}
	...
	private static class ExpandoTableModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					expandoTableModelImpl, attributeNames, original);
			}
			...
			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |=
					expandoTableModelImpl.getColumnBitmask(attributeName);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					...
					arguments[i] = expandoTableModelImpl.getAttributeValue(
						attributeName);
				}
			}
