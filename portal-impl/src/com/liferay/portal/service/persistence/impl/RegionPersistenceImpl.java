...
public class RegionPersistenceImpl
	...
	public void afterPropertiesSet() {
		...
		_finderPathWithPaginationFindByCountryId = _createFinderPath(
			...
			"findByCountryId", new String[] {"countryId"});
		...
		_finderPathCountByCountryId = _createFinderPath(
			...
			"countByCountryId", new String[] {"countryId"});
		...
		_finderPathWithPaginationFindByC_A = _createFinderPath(
			RegionImpl.class, FINDER_CLASS_NAME_LIST_WITH_PAGINATION,
			"findByC_A", new String[] {"countryId", "active"});
	...
	private static class RegionModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(regionModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			RegionModelImpl regionModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] = regionModelImpl.getOriginalAttributeValue(
						attributeName);
				}
				else {
					arguments[i] = regionModelImpl.getAttributeValue(
						attributeName);
				}
			}
