...
public class OAuthTokenPersistenceImpl
	...
	public void afterPropertiesSet() {
		...
		_finderPathWithPaginationFindByG_S = _createFinderPath(
			OAuthTokenImpl.class, FINDER_CLASS_NAME_LIST_WITH_PAGINATION,
			"findByG_S", new String[] {"gadgetKey", "serviceName"});
		...
		_finderPathCountByG_S = _createFinderPath(
			...
			new String[] {"gadgetKey", "serviceName"});
	...
	private static class OAuthTokenModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(oAuthTokenModelImpl, attributeNames, original);
			}

			long finderPathColumnBitmask = 0;

			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |= oAuthTokenModelImpl.getColumnBitmask(
					attributeName);
			}
		...
		private Object[] _getValue(
			OAuthTokenModelImpl oAuthTokenModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						oAuthTokenModelImpl.getOriginalAttributeValue(
							attributeName);
				}
				else {
					arguments[i] = oAuthTokenModelImpl.getAttributeValue(
						attributeName);
				}
			}
