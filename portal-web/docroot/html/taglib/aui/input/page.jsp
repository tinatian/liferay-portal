...
<c:choose>
	<c:when test='<%= type.equals("hidden") %>'>
		...
		<input id="<%= namespace + id %>" name="<%= namespace + name %>" <%= Validator.isNotNull(title) ? "title=\"" + LanguageUtil.get(resourceBundle, title) + "\"" : StringPool.BLANK %> type="<%= Validator.isNull(type) ? "text" : type %>" <%= "value=\"" + HtmlUtil.escapeAttribute(valueString) + "\""%> />
	...
	<c:otherwise>
		...
		<c:if test='<%= !wrappedField && useInputWrapper %>'>
		...
		<c:if test='<%= !type.equals("assetCategories") && Validator.isNotNull(labelContent) %>'>
			<label <%= labelTag %>>
		...
		<c:choose>
			...
			<c:otherwise>
				...
				if (!ignoreRequestValue && (Validator.isNull(type) || ArrayUtil.contains(_TYPES, type))) {
