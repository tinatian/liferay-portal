	...
	public void afterPropertiesSet() {
		...
		_argumentsResolverServiceRegistration = registry.registerService(
			...
			HashMapBuilder.<String, Object>put(
				"model.class.name", UserGroup.class.getName()
			).build());
		...
		_finderPathWithPaginationFindByC_P = _createFinderPath(
			UserGroupImpl.class, FINDER_CLASS_NAME_LIST_WITH_PAGINATION,
			"findByC_P", new String[] {"companyId", "parentUserGroupId"});
		...
		_finderPathCountByC_P = _createFinderPath(
			...
			new String[] {"companyId", "parentUserGroupId"});
	...
	private static class UserGroupModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(userGroupModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			UserGroupModelImpl userGroupModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] = userGroupModelImpl.getOriginalAttributeValue(
						attributeName);
				}
				else {
					arguments[i] = userGroupModelImpl.getAttributeValue(
						attributeName);
				}
			}
