	...
	public void afterPropertiesSet() {
		...
		_argumentsResolverServiceRegistration = _bundleContext.registerService(
			...
			MapUtil.singletonDictionary(
				"model.class.name", UADPartialEntry.class.getName()));
	...
	private static class UADPartialEntryModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					uadPartialEntryModelImpl, attributeNames, original);
			}
			...
			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |=
					uadPartialEntryModelImpl.getColumnBitmask(attributeName);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						uadPartialEntryModelImpl.getOriginalAttributeValue(
							attributeName);
				}
				else {
					arguments[i] = uadPartialEntryModelImpl.getAttributeValue(
						attributeName);
				}
			}
