...
public class BigDecimalEntryPersistenceImpl
	...
	public void afterPropertiesSet() {
		...
		_finderPathWithPaginationFindByBigDecimalValue = _createFinderPath(
			BigDecimalEntryImpl.class, FINDER_CLASS_NAME_LIST_WITH_PAGINATION,
			"findByBigDecimalValue", new String[] {"bigDecimalValue"});
	...
	private static class BigDecimalEntryModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					bigDecimalEntryModelImpl, attributeNames, original);
			}
			...
			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |=
					bigDecimalEntryModelImpl.getColumnBitmask(attributeName);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						bigDecimalEntryModelImpl.getOriginalAttributeValue(
							attributeName);
				}
				else {
					arguments[i] = bigDecimalEntryModelImpl.getAttributeValue(
						attributeName);
				}
			}
