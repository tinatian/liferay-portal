...
public class CalendarPersistenceImpl
	...
	public void activate(BundleContext bundleContext) {
		...
		_finderPathWithPaginationFindByG_C = _createFinderPath(
			CalendarImpl.class, FINDER_CLASS_NAME_LIST_WITH_PAGINATION,
			"findByG_C", new String[] {"groupId", "calendarResourceId"});
		...
		_finderPathCountByG_C = _createFinderPath(
			...
			new String[] {"groupId", "calendarResourceId"});
	...
	private static class CalendarModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(calendarModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			CalendarModelImpl calendarModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] = calendarModelImpl.getOriginalAttributeValue(
						attributeName);
				}
				else {
					arguments[i] = calendarModelImpl.getAttributeValue(
						attributeName);
				}
			}
