...
public class FolderPersistenceImpl
	...
	public void clearCache(Folder folder) {
		...
		entityCache.removeResult(
			entityCacheEnabled, FolderImpl.class, folder,
	...
	public Folder updateImpl(Folder folder) {
		...
		entityCache.putResult(
			entityCacheEnabled, FolderImpl.class, folder, false,
			_columnBitmaskEnabled);
	...
	public void activate(BundleContext bundleContext) {
		...
		_finderPathWithPaginationFindAll = new FinderPath(
			finderCacheEnabled, FolderImpl.class,
		...
		_finderPathFetchByA_F = new FinderPath(
			finderCacheEnabled, FolderImpl.class, FINDER_CLASS_NAME_ENTITY,
			"fetchByA_F",
			new String[] {Long.class.getName(), String.class.getName()},
			_ARGUMENTS_BIFUNCTION_MAP.get("A_F"),
			_ORIGINAL_ARGUMENTS_BIFUNCTION_MAP.get("A_F"));

		_serviceRegistrations.add(
			bundleContext.registerService(
				FinderPath.class, _finderPathFetchByA_F,
				MapUtil.singletonDictionary(
		...
		_serviceRegistrations.add(
			bundleContext.registerService(
				FinderPath.class, _finderPathCountByA_F,
				MapUtil.singletonDictionary(
					"cache.name", FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION)));
	}
	...
	static {
		_ARGUMENTS_BIFUNCTION_MAP.put(
			...
			(baseModel, checkColumns) -> {
				...
				if (!checkColumns ||
					((folderModelImpl.getColumnBitmask() &
					  _FINDER_PATH_BITMASK_MAP.get("AccountId")) != 0)) {

					return new Object[] {folderModelImpl.getAccountId()};
				}
		...
		_ARGUMENTS_BIFUNCTION_MAP.put(
			...
			(baseModel, checkColumns) -> {
				...
				if (!checkColumns ||
					((folderModelImpl.getColumnBitmask() &
					  _FINDER_PATH_BITMASK_MAP.get("A_F")) != 0)) {
