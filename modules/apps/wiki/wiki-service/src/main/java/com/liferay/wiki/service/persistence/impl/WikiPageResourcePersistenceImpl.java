...
public class WikiPageResourcePersistenceImpl
	...
	public void clearCache(WikiPageResource wikiPageResource) {
		...
		entityCache.removeResult(
			entityCacheEnabled, WikiPageResourceImpl.class, wikiPageResource,
	...
	public void clearCache(List<WikiPageResource> wikiPageResources) {
		...
		for (WikiPageResource wikiPageResource : wikiPageResources) {
			entityCache.removeResult(
				entityCacheEnabled, WikiPageResourceImpl.class,
				wikiPageResource, _columnBitmaskEnabled);
		}
	}
	...
	public void activate(BundleContext bundleContext) {
		...
		_finderPathWithPaginationFindAll = new FinderPath(
			finderCacheEnabled, WikiPageResourceImpl.class,
		...
		_serviceRegistrations.add(
			bundleContext.registerService(
				FinderPath.class, _finderPathFetchByN_T,
				MapUtil.singletonDictionary(
					"cache.name", FINDER_CLASS_NAME_ENTITY)));
	...
	static {
		_FINDER_PATH_BITMASK_MAP.put(
			"Uuid", WikiPageResourceModelImpl.UUID_COLUMN_BITMASK);
	...
	static {
		_ARGUMENTS_BIFUNCTION_MAP.put(
			...
			(baseModel, checkColumns) -> {
				...
				if (!checkColumns ||
					((wikiPageResourceModelImpl.getColumnBitmask() &
					  _FINDER_PATH_BITMASK_MAP.get("Uuid")) != 0)) {

					return new Object[] {wikiPageResourceModelImpl.getUuid()};
				}
