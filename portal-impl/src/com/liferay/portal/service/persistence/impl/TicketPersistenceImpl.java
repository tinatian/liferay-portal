...
public class TicketPersistenceImpl
	...
	public void afterPropertiesSet() {
		...
		_finderPathWithPaginationFindByC_C_C_T = _createFinderPath(
			TicketImpl.class, FINDER_CLASS_NAME_LIST_WITH_PAGINATION,
			"findByC_C_C_T",
			new String[] {"companyId", "classNameId", "classPK", "type"});
	...
	private static class TicketModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(ticketModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			TicketModelImpl ticketModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] = ticketModelImpl.getOriginalAttributeValue(
						attributeName);
				}
				else {
					arguments[i] = ticketModelImpl.getAttributeValue(
						attributeName);
				}
			}
