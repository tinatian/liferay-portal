...
public class KBFolderPersistenceImpl
	...
	public void activate(BundleContext bundleContext) {
		...
		_finderPathWithPaginationFindByG_P = _createFinderPath(
			KBFolderImpl.class, FINDER_CLASS_NAME_LIST_WITH_PAGINATION,
			"findByG_P", new String[] {"groupId", "parentKBFolderId"});
		...
		_finderPathCountByG_P = _createFinderPath(
			...
			new String[] {"groupId", "parentKBFolderId"});
	...
	private static class KBFolderModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(kbFolderModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			KBFolderModelImpl kbFolderModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] = kbFolderModelImpl.getOriginalAttributeValue(
						attributeName);
				}
				else {
					arguments[i] = kbFolderModelImpl.getAttributeValue(
						attributeName);
				}
			}
