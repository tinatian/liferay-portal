...
public class ReleasePersistenceImpl
	...
	public void afterPropertiesSet() {
		...
		_finderPathFetchByServletContextName = _createFinderPath(
			...
			"fetchByServletContextName", new String[] {"servletContextName"});

		_finderPathCountByServletContextName = _createFinderPath(
			Long.class, FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION,
			"countByServletContextName", new String[] {"servletContextName"});
	}
	...
	private static class ReleaseModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(releaseModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			ReleaseModelImpl releaseModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] = releaseModelImpl.getOriginalAttributeValue(
						attributeName);
				}
				else {
					arguments[i] = releaseModelImpl.getAttributeValue(
						attributeName);
				}
			}
