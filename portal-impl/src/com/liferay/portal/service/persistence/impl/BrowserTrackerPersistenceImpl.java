...
public class BrowserTrackerPersistenceImpl
	...
	public void afterPropertiesSet() {
		...
		_argumentsResolverServiceRegistration = registry.registerService(
			ArgumentsResolver.class, new BrowserTrackerModelArgumentsResolver(),
			HashMapBuilder.<String, Object>put(
				"model.class.name", BrowserTracker.class.getName()
			).build());
	...
	private static class BrowserTrackerModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					browserTrackerModelImpl, attributeNames, original);
			}
			...
			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |=
					browserTrackerModelImpl.getColumnBitmask(attributeName);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						browserTrackerModelImpl.getOriginalAttributeValue(
							attributeName);
				}
				else {
					arguments[i] = browserTrackerModelImpl.getAttributeValue(
						attributeName);
				}
			}
