	...
	public void activate(BundleContext bundleContext) {
		...
		_argumentsResolverServiceRegistration = _bundleContext.registerService(
			...
			MapUtil.singletonDictionary(
				"model.class.name", BatchEngineImportTask.class.getName()));
		...
		_finderPathCountByExecuteStatus = _createFinderPath(
			Long.class, FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION,
			"countByExecuteStatus", new String[] {"executeStatus"});
	}
	...
	private static class BatchEngineImportTaskModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					batchEngineImportTaskModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					...
					arguments[i] =
						batchEngineImportTaskModelImpl.getAttributeValue(
							attributeName);
				}
			}
