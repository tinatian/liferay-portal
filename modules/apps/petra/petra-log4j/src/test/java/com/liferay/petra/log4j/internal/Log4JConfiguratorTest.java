...
public class Log4JConfiguratorTest {
	...
	public void testGetLogLevelStrings() {
		...
		Assert.assertEquals(
			"The level should be WARN by configuration", _WARN,
			logLevelStrings.get(loggerName));

		// LoggerConfig.getLevel() uses parent level if itsself level is null
		...
		Assert.assertEquals(
			"The level should use rootLogger's level ERROR", _ERROR,
			logLevelStrings.get(loggerName));
	}
	...
	private String _generateXMLConfigurationContent(
		...
		StringBundler sb = new StringBundler(10 + (10 * appenderTypes.length));
