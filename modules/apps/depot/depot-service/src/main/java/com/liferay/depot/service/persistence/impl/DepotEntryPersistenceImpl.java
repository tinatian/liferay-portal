	...
	private static class DepotEntryModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(depotEntryModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			DepotEntryModelImpl depotEntryModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						depotEntryModelImpl.getOriginalAttributeValue(
							attributeName);
				}
				else {
					arguments[i] = depotEntryModelImpl.getAttributeValue(
						attributeName);
				}
			}
