	...
	private static class AccountEntryModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					accountEntryModelImpl, attributeNames, original);
			}
			...
			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |=
					accountEntryModelImpl.getColumnBitmask(attributeName);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						accountEntryModelImpl.getOriginalAttributeValue(
							attributeName);
				}
				else {
					arguments[i] = accountEntryModelImpl.getAttributeValue(
						attributeName);
				}
			}
