...
public class PortletItemPersistenceImpl
	...
	public void afterPropertiesSet() {
		...
		_finderPathWithPaginationFindByG_P_C = _createFinderPath(
			PortletItemImpl.class, FINDER_CLASS_NAME_LIST_WITH_PAGINATION,
			"findByG_P_C",
			new String[] {"groupId", "portletId", "classNameId"});
	...
	private static class PortletItemModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					portletItemModelImpl, attributeNames, original);
			}
			...
			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |=
					portletItemModelImpl.getColumnBitmask(attributeName);
			}
		...
		private Object[] _getValue(
			PortletItemModelImpl portletItemModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					...
					arguments[i] = portletItemModelImpl.getAttributeValue(
						attributeName);
				}
			}
