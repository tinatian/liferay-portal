...
public class ModifiedFacetDisplayContextBuilder implements Serializable {
	...
	private String _getCustomRangeURL() {
		...
		String rangeURL = _httpImpl.removeParameter(_currentURL, "modified");

		rangeURL = _httpImpl.removeParameter(
			rangeURL, _paginationStartParameterName);

		rangeURL = _httpImpl.setParameter(rangeURL, "modifiedFrom", from);

		return _httpImpl.setParameter(rangeURL, "modifiedTo", to);
	}

	private String _getLabeledRangeURL(String label) {
		String rangeURL = _httpImpl.removeParameter(
			_currentURL, "modifiedFrom");

		rangeURL = _httpImpl.removeParameter(rangeURL, "modifiedTo");

		rangeURL = _httpImpl.removeParameter(
			rangeURL, _paginationStartParameterName);

		return _httpImpl.setParameter(rangeURL, "modified", label);
	}
