 ...
 * @author Tina Tian
 */
class DynamicPortalCacheManager<K extends Serializable, V>
	...
	public PortalCache<K, V> getPortalCache(
		...
		DynamicPortalCache<K, V> previousDynamicPortalCache =
			_dynamicPortalCaches.putIfAbsent(
				portalCacheName, dynamicPortalCache);

		if (previousDynamicPortalCache != null) {
			return previousDynamicPortalCache;
		}
	...
	public Set<PortalCacheManagerListener> getPortalCacheManagerListeners() {
		if (_portalCacheManager == _DUMMY_PORTAL_CACHE_MANAGER) {
	...
	protected void setPortalCacheManager(
		...
		for (DynamicPortalCache<K, V> dynamicPortalCache :
			...
			PortalCache<K, V> portalCache = portalCacheManager.getPortalCache(
				dynamicPortalCache.getPortalCacheName(),
				dynamicPortalCache.isBlocking(), dynamicPortalCache.isMVCC());

			dynamicPortalCache.setPortalCache(portalCache);
		}
