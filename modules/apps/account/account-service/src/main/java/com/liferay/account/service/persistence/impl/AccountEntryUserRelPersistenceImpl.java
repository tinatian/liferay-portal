...
public class AccountEntryUserRelPersistenceImpl
	...
	public void activate(BundleContext bundleContext) {
		...
		_finderPathFetchByAEI_AUI = _createFinderPath(
			...
			"fetchByAEI_AUI", new String[] {"accountEntryId", "accountUserId"});

		_finderPathCountByAEI_AUI = _createFinderPath(
			Long.class, FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION,
			"countByAEI_AUI", new String[] {"accountEntryId", "accountUserId"});
	}
	...
	private static class AccountEntryUserRelModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					accountEntryUserRelModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						accountEntryUserRelModelImpl.getOriginalAttributeValue(
							attributeName);
				}
				else {
					arguments[i] =
						accountEntryUserRelModelImpl.getAttributeValue(
							attributeName);
				}
			}
