	...
	public void activate(BundleContext bundleContext) {
		...
		_argumentsResolverServiceRegistration = _bundleContext.registerService(
			...
			MapUtil.singletonDictionary(
				"model.class.name", ReadingTimeEntry.class.getName()));
		...
		_finderPathFetchByG_C_C = _createFinderPath(
			ReadingTimeEntryImpl.class, FINDER_CLASS_NAME_ENTITY,
			"fetchByG_C_C", new String[] {"groupId", "classNameId", "classPK"});
	...
	private static class ReadingTimeEntryModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					readingTimeEntryModelImpl, attributeNames, original);
			}
			...
			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |=
					readingTimeEntryModelImpl.getColumnBitmask(attributeName);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						readingTimeEntryModelImpl.getOriginalAttributeValue(
							attributeName);
				}
				else {
					arguments[i] = readingTimeEntryModelImpl.getAttributeValue(
						attributeName);
				}
			}
