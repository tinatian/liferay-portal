...
public class SamlIdpSpConnectionPersistenceImpl
	...
	public void activate(BundleContext bundleContext) {
		...
		_finderPathFetchByC_SSEI = _createFinderPath(
			...
			"fetchByC_SSEI", new String[] {"companyId", "samlSpEntityId"});

		_finderPathCountByC_SSEI = _createFinderPath(
			Long.class, FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION,
			"countByC_SSEI", new String[] {"companyId", "samlSpEntityId"});
	}
	...
	private static class SamlIdpSpConnectionModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					samlIdpSpConnectionModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					...
					arguments[i] =
						samlIdpSpConnectionModelImpl.getAttributeValue(
							attributeName);
				}
			}
