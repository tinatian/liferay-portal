	...
	public void afterPropertiesSet() {
		...
		_argumentsResolverServiceRegistration = registry.registerService(
			...
			HashMapBuilder.<String, Object>put(
				"model.class.name", AnnouncementsDelivery.class.getName()
			).build());
		...
		_finderPathFetchByU_T = _createFinderPath(
			...
			"fetchByU_T", new String[] {"userId", "type"});
	...
	private static class AnnouncementsDeliveryModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					announcementsDeliveryModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					...
					arguments[i] =
						announcementsDeliveryModelImpl.getAttributeValue(
							attributeName);
				}
			}
