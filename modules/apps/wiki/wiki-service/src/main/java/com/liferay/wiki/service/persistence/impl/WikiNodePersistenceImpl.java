	...
	private static class WikiNodeModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(wikiNodeModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			WikiNodeModelImpl wikiNodeModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] = wikiNodeModelImpl.getOriginalAttributeValue(
						attributeName);
				}
				else {
					arguments[i] = wikiNodeModelImpl.getAttributeValue(
						attributeName);
				}
			}
