...
public class PowwowServerPersistenceImpl
	...
	public void afterPropertiesSet() {
		...
		_finderPathWithPaginationFindByPT_A = _createFinderPath(
			PowwowServerImpl.class, FINDER_CLASS_NAME_LIST_WITH_PAGINATION,
			"findByPT_A", new String[] {"providerType", "active"});
		...
		_finderPathCountByPT_A = _createFinderPath(
			...
			"countByPT_A", new String[] {"providerType", "active"});
	}
	...
	private static class PowwowServerModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					powwowServerModelImpl, attributeNames, original);
			}
			...
			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |=
					powwowServerModelImpl.getColumnBitmask(attributeName);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						powwowServerModelImpl.getOriginalAttributeValue(
							attributeName);
				}
				else {
					arguments[i] = powwowServerModelImpl.getAttributeValue(
						attributeName);
				}
			}
