...
public class VirtualHostPersistenceImpl
	...
	public void afterPropertiesSet() {
		...
		_finderPathCountByHostname = _createFinderPath(
			...
			"countByHostname", new String[] {"hostname"});

		_finderPathWithPaginationFindByC_L = _createFinderPath(
			VirtualHostImpl.class, FINDER_CLASS_NAME_LIST_WITH_PAGINATION,
			"findByC_L", new String[] {"companyId", "layoutSetId"});
		...
		_finderPathCountByC_L = _createFinderPath(
			...
			new String[] {"companyId", "layoutSetId"});
	...
	private static class VirtualHostModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					virtualHostModelImpl, attributeNames, original);
			}
			...
			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |=
					virtualHostModelImpl.getColumnBitmask(attributeName);
			}
		...
		private Object[] _getValue(
			VirtualHostModelImpl virtualHostModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						virtualHostModelImpl.getOriginalAttributeValue(
							attributeName);
				}
				else {
					arguments[i] = virtualHostModelImpl.getAttributeValue(
						attributeName);
				}
			}
