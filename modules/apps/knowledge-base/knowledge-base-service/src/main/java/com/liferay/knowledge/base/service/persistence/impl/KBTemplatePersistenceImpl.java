...
public class KBTemplatePersistenceImpl
	...
	public void cacheResult(KBTemplate kbTemplate) {
		entityCache.putResult(
			...
			new Object[] {
				...
				((KBTemplateModelImpl)kbTemplate).getColumnBitmask()
			});
	...
	public KBTemplate updateImpl(KBTemplate kbTemplate) {
		...
		entityCache.putResult(
			entityCacheEnabled, KBTemplateImpl.class,
			kbTemplateModelImpl.getPrimaryKey(), kbTemplateModelImpl, false,
			new Object[] {
				_columnBitmaskEnabled, kbTemplateModelImpl.getColumnBitmask()
			});
	...
	private FinderPath _getFinderPath(String cacheName, String methodName) {
		...
		Map.Entry<FinderPath, ServiceRegistration<FinderPath>> entry =
			_finderPathMap.computeIfAbsent(
				...
				key -> {
					Class<?> returnClass = KBTemplateImpl.class;
