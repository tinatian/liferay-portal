	...
	private static class KaleoTimerModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(kaleoTimerModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			KaleoTimerModelImpl kaleoTimerModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					...
					arguments[i] = kaleoTimerModelImpl.getAttributeValue(
						attributeName);
				}
			}
