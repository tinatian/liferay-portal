...
public class OAuthApplicationPersistenceImpl
	...
	public void cacheResult(OAuthApplication oAuthApplication) {
		entityCache.putResult(
			...
			oAuthApplication.getPrimaryKey(), oAuthApplication);
	...
	public void clearCache(OAuthApplication oAuthApplication) {
		...
		entityCache.removeResult(
			entityCacheEnabled, OAuthApplicationImpl.class, oAuthApplication,
	...
	public void clearCache(List<OAuthApplication> oAuthApplications) {
		...
		for (OAuthApplication oAuthApplication : oAuthApplications) {
			entityCache.removeResult(
				entityCacheEnabled, OAuthApplicationImpl.class,
				oAuthApplication, _columnBitmaskEnabled);
		}
	}
	...
	public void activate(BundleContext bundleContext) {
		...
		_finderPathWithPaginationFindAll = new FinderPath(
			finderCacheEnabled, OAuthApplicationImpl.class,
		...
		_finderPathFetchByConsumerKey = new FinderPath(
			...
			_ARGUMENTS_BIFUNCTION_MAP.get("ConsumerKey"),
			_ORIGINAL_ARGUMENTS_BIFUNCTION_MAP.get("ConsumerKey"));

		_serviceRegistrations.add(
			bundleContext.registerService(
				FinderPath.class, _finderPathFetchByConsumerKey,
				MapUtil.singletonDictionary(
		...
		_serviceRegistrations.add(
			bundleContext.registerService(
				FinderPath.class, _finderPathCountByConsumerKey,
				MapUtil.singletonDictionary(
					"cache.name", FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION)));
	...
	static {
		_FINDER_PATH_BITMASK_MAP.put(
			"CompanyId", OAuthApplicationModelImpl.COMPANYID_COLUMN_BITMASK);

		_FINDER_PATH_BITMASK_MAP.put(
			"UserId", OAuthApplicationModelImpl.USERID_COLUMN_BITMASK);
	...
	static {
		_ARGUMENTS_BIFUNCTION_MAP.put(
			...
			(baseModel, checkColumns) -> {
				...
				if (!checkColumns ||
					((oAuthApplicationModelImpl.getColumnBitmask() &
		...
		_ARGUMENTS_BIFUNCTION_MAP.put(
			...
			(baseModel, checkColumns) -> {
				...
				if (!checkColumns ||
					...
					return new Object[] {oAuthApplicationModelImpl.getUserId()};
				}
		...
		_ARGUMENTS_BIFUNCTION_MAP.put(
			...
			(baseModel, checkColumns) -> {
				...
				if (!checkColumns ||
					((oAuthApplicationModelImpl.getColumnBitmask() &
					  _FINDER_PATH_BITMASK_MAP.get("ConsumerKey")) != 0)) {
