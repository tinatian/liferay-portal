...
public class DispatchLogPersistenceImpl
	...
	public void activate(BundleContext bundleContext) {
		...
		_finderPathWithPaginationFindByDispatchTriggerId = _createFinderPath(
			DispatchLogImpl.class, FINDER_CLASS_NAME_LIST_WITH_PAGINATION,
			"findByDispatchTriggerId", new String[] {"dispatchTriggerId"});
		...
		_finderPathCountByDispatchTriggerId = _createFinderPath(
			...
			"countByDispatchTriggerId", new String[] {"dispatchTriggerId"});

		_finderPathWithPaginationFindByDTI_S = _createFinderPath(
			DispatchLogImpl.class, FINDER_CLASS_NAME_LIST_WITH_PAGINATION,
			"findByDTI_S", new String[] {"dispatchTriggerId", "status"});
		...
		_finderPathCountByDTI_S = _createFinderPath(
			Long.class, FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION,
			"countByDTI_S", new String[] {"dispatchTriggerId", "status"});
	}
	...
	private static class DispatchLogModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					dispatchLogModelImpl, attributeNames, original);
			}
			...
			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |=
					dispatchLogModelImpl.getColumnBitmask(attributeName);
			}
		...
		private Object[] _getValue(
			DispatchLogModelImpl dispatchLogModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					...
					arguments[i] = dispatchLogModelImpl.getAttributeValue(
						attributeName);
				}
			}
