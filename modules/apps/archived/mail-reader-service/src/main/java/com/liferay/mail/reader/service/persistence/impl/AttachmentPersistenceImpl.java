...
public class AttachmentPersistenceImpl
	...
	public void clearCache(Attachment attachment) {
		...
		entityCache.removeResult(
			entityCacheEnabled, AttachmentImpl.class, attachment,
	...
	public Attachment updateImpl(Attachment attachment) {
		...
		entityCache.putResult(
			entityCacheEnabled, AttachmentImpl.class, attachment, false,
			_columnBitmaskEnabled);
	...
	public void activate(BundleContext bundleContext) {
		...
		_finderPathWithPaginationFindAll = new FinderPath(
			finderCacheEnabled, AttachmentImpl.class,
		...
		_finderPathWithoutPaginationFindByMessageId = new FinderPath(
			...
			_ARGUMENTS_BIFUNCTION_MAP.get("MessageId"),
			_ORIGINAL_ARGUMENTS_BIFUNCTION_MAP.get("MessageId"));

		_serviceRegistrations.add(
			bundleContext.registerService(
				FinderPath.class, _finderPathWithoutPaginationFindByMessageId,
				MapUtil.singletonDictionary(
					"cache.name", FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION)));
		...
		_serviceRegistrations.add(
			bundleContext.registerService(
				FinderPath.class, _finderPathCountByMessageId,
				MapUtil.singletonDictionary(
					"cache.name", FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION)));
	}
	...
	static {
		_FINDER_PATH_BITMASK_MAP.put(
			"MessageId", AttachmentModelImpl.MESSAGEID_COLUMN_BITMASK);
	}
	...
	static {
		_ARGUMENTS_BIFUNCTION_MAP.put(
			...
			(baseModel, checkColumns) -> {
				...
				if (!checkColumns ||
					((attachmentModelImpl.getColumnBitmask() &
					  _FINDER_PATH_BITMASK_MAP.get("MessageId")) != 0)) {

					return new Object[] {attachmentModelImpl.getMessageId()};
				}
