	...
	public void afterPropertiesSet() {
		...
		_argumentsResolverServiceRegistration = registry.registerService(
			...
			HashMapBuilder.<String, Object>put(
				"model.class.name", WebDAVProps.class.getName()
			).build());
	...
	private static class WebDAVPropsModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					webDAVPropsModelImpl, attributeNames, original);
			}
			...
			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |=
					webDAVPropsModelImpl.getColumnBitmask(attributeName);
			}
		...
		private Object[] _getValue(
			WebDAVPropsModelImpl webDAVPropsModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						webDAVPropsModelImpl.getOriginalAttributeValue(
							attributeName);
				}
				else {
					arguments[i] = webDAVPropsModelImpl.getAttributeValue(
						attributeName);
				}
			}
