	...
	public void afterPropertiesSet() {
		...
		_argumentsResolverServiceRegistration = registry.registerService(
			...
			HashMapBuilder.<String, Object>put(
				"model.class.name", UserIdMapper.class.getName()
			).build());
		...
		_finderPathFetchByT_E = _createFinderPath(
			...
			new String[] {"type", "externalUserId"});
	...
	private static class UserIdMapperModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					userIdMapperModelImpl, attributeNames, original);
			}
			...
			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |=
					userIdMapperModelImpl.getColumnBitmask(attributeName);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						userIdMapperModelImpl.getOriginalAttributeValue(
							attributeName);
				}
				else {
					arguments[i] = userIdMapperModelImpl.getAttributeValue(
						attributeName);
				}
			}
