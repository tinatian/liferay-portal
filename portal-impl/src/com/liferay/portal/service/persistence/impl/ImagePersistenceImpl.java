	...
	private static class ImageModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(imageModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			ImageModelImpl imageModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] = imageModelImpl.getOriginalAttributeValue(
						attributeName);
				}
				else {
					arguments[i] = imageModelImpl.getAttributeValue(
						attributeName);
				}
			}
