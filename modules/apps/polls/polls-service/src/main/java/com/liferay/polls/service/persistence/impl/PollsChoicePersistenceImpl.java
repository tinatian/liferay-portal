	...
	private static class PollsChoiceModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					pollsChoiceModelImpl, attributeNames, original);
			}
			...
			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |=
					pollsChoiceModelImpl.getColumnBitmask(attributeName);
			}
		...
		private Object[] _getValue(
			PollsChoiceModelImpl pollsChoiceModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						pollsChoiceModelImpl.getOriginalAttributeValue(
							attributeName);
				}
				else {
					arguments[i] = pollsChoiceModelImpl.getAttributeValue(
						attributeName);
				}
			}
