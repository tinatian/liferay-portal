	...
	public void afterPropertiesSet() {
		...
		_argumentsResolverServiceRegistration = registry.registerService(
			...
			HashMapBuilder.<String, Object>put(
				"model.class.name", ExpandoRow.class.getName()
			).build());
	...
	private static class ExpandoRowModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(expandoRowModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			ExpandoRowModelImpl expandoRowModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						expandoRowModelImpl.getOriginalAttributeValue(
							attributeName);
				}
				else {
					arguments[i] = expandoRowModelImpl.getAttributeValue(
						attributeName);
				}
			}
