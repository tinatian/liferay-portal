...
<#if dependencyInjectorDS>
	@Component(
		immediate = true,
		<#if serviceBuilder.isVersionGTE_7_3_0() && !serviceBuilder.isVersionGTE_7_4_0()>
			property = "model.class.name=${entity.name}.class.getName()",
		</#if>
		service = {${entity.name}ModelArgumentsResolver.class, ArgumentsResolver.class}
	)
</#if>
public class ${entity.name}ModelArgumentsResolver implements ArgumentsResolver {
