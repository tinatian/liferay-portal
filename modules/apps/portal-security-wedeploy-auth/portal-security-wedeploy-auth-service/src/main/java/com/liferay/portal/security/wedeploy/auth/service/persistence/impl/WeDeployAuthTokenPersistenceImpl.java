...
public class WeDeployAuthTokenPersistenceImpl
	...
	public void clearCache(WeDeployAuthToken weDeployAuthToken) {
		...
		entityCache.removeResult(
			entityCacheEnabled, WeDeployAuthTokenImpl.class, weDeployAuthToken,
	...
	public void clearCache(List<WeDeployAuthToken> weDeployAuthTokens) {
		...
		for (WeDeployAuthToken weDeployAuthToken : weDeployAuthTokens) {
			entityCache.removeResult(
				entityCacheEnabled, WeDeployAuthTokenImpl.class,
				weDeployAuthToken, _columnBitmaskEnabled);
		}
	}
	...
	public void activate(BundleContext bundleContext) {
		...
		_finderPathWithPaginationFindAll = new FinderPath(
			finderCacheEnabled, WeDeployAuthTokenImpl.class,
		...
		_finderPathFetchByCI_T_T = new FinderPath(
			...
			_ARGUMENTS_BIFUNCTION_MAP.get("CI_T_T"),
			_ORIGINAL_ARGUMENTS_BIFUNCTION_MAP.get("CI_T_T"));

		_serviceRegistrations.add(
			bundleContext.registerService(
				FinderPath.class, _finderPathFetchByCI_T_T,
				MapUtil.singletonDictionary(
					"cache.name", FINDER_CLASS_NAME_ENTITY)));
		...
		_serviceRegistrations.add(
			bundleContext.registerService(
				FinderPath.class, _finderPathCountByCI_T_T,
				MapUtil.singletonDictionary(
					"cache.name", FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION)));
	}
	...
	static {
		...
		_ARGUMENTS_BIFUNCTION_MAP.put(
			...
			(baseModel, checkColumns) -> {
				...
				if (!checkColumns ||
					((weDeployAuthTokenModelImpl.getColumnBitmask() &
					  _FINDER_PATH_BITMASK_MAP.get("CI_T_T")) != 0)) {
