...
public class DispatchLogPersistenceImpl
	...
	public void clearCache(DispatchLog dispatchLog) {
		...
		entityCache.removeResult(
			entityCacheEnabled, DispatchLogImpl.class, dispatchLog,
	...
	public DispatchLog updateImpl(DispatchLog dispatchLog) {
		...
		entityCache.putResult(
			entityCacheEnabled, DispatchLogImpl.class, dispatchLog, false,
			_columnBitmaskEnabled);
	...
	public void activate(BundleContext bundleContext) {
		...
		_finderPathWithPaginationFindAll = new FinderPath(
			finderCacheEnabled, DispatchLogImpl.class,
		...
		_finderPathWithoutPaginationFindByDispatchTriggerId = new FinderPath(
			...
			_ARGUMENTS_BIFUNCTION_MAP.get("DispatchTriggerId"),
			_ORIGINAL_ARGUMENTS_BIFUNCTION_MAP.get("DispatchTriggerId"));
		...
		_serviceRegistrations.add(
			bundleContext.registerService(
				FinderPath.class, _finderPathCountByDispatchTriggerId,
				MapUtil.singletonDictionary(
					"cache.name", FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION)));
		...
		_finderPathWithoutPaginationFindByDTI_S = new FinderPath(
			...
			_ARGUMENTS_BIFUNCTION_MAP.get("DTI_S"),
			_ORIGINAL_ARGUMENTS_BIFUNCTION_MAP.get("DTI_S"));

		_serviceRegistrations.add(
			bundleContext.registerService(
				FinderPath.class, _finderPathWithoutPaginationFindByDTI_S,
				MapUtil.singletonDictionary(
					"cache.name", FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION)));
		...
		_serviceRegistrations.add(
			bundleContext.registerService(
				FinderPath.class, _finderPathCountByDTI_S,
				MapUtil.singletonDictionary(
					"cache.name", FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION)));
	}
	...
	static {
		_ARGUMENTS_BIFUNCTION_MAP.put(
			...
			(baseModel, checkColumns) -> {
				...
				if (!checkColumns ||
					((dispatchLogModelImpl.getColumnBitmask() &
					  _FINDER_PATH_BITMASK_MAP.get("DispatchTriggerId")) !=
						  0)) {
		...
		_ARGUMENTS_BIFUNCTION_MAP.put(
			...
			(baseModel, checkColumns) -> {
				...
				if (!checkColumns ||
					((dispatchLogModelImpl.getColumnBitmask() &
					  _FINDER_PATH_BITMASK_MAP.get("DTI_S")) != 0)) {
