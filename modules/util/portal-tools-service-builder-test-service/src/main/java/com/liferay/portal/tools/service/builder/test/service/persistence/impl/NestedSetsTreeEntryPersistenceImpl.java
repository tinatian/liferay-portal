...
public class NestedSetsTreeEntryPersistenceImpl
	...
	private static class NestedSetsTreeEntryModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			long columnBitmask =
				nestedSetsTreeEntryModelImpl.getColumnBitmask();

			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					nestedSetsTreeEntryModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						nestedSetsTreeEntryModelImpl.getOriginalAttributeValue(
							attributeName);
				}
				else {
					arguments[i] =
						nestedSetsTreeEntryModelImpl.getAttributeValue(
							attributeName);
				}
			}
