	...
	public void afterPropertiesSet() {
		...
		_argumentsResolverServiceRegistration = registry.registerService(
			...
			HashMapBuilder.<String, Object>put(
				"model.class.name", UserTracker.class.getName()
			).build());
		...
		_finderPathWithPaginationFindBySessionId = _createFinderPath(
			UserTrackerImpl.class, FINDER_CLASS_NAME_LIST_WITH_PAGINATION,
			"findBySessionId", new String[] {"sessionId"});
		...
		_finderPathCountBySessionId = _createFinderPath(
			Long.class, FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION,
			"countBySessionId", new String[] {"sessionId"});
	}
	...
	private static class UserTrackerModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					userTrackerModelImpl, attributeNames, original);
			}
			...
			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |=
					userTrackerModelImpl.getColumnBitmask(attributeName);
			}
		...
		private Object[] _getValue(
			UserTrackerModelImpl userTrackerModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						userTrackerModelImpl.getOriginalAttributeValue(
							attributeName);
				}
				else {
					arguments[i] = userTrackerModelImpl.getAttributeValue(
						attributeName);
				}
			}
