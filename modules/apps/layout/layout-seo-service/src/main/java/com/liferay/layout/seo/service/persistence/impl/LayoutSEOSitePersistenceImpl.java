	...
	public void activate(BundleContext bundleContext) {
		...
		_argumentsResolverServiceRegistration = _bundleContext.registerService(
			...
			MapUtil.singletonDictionary(
				"model.class.name", LayoutSEOSite.class.getName()));
	...
	private static class LayoutSEOSiteModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					layoutSEOSiteModelImpl, attributeNames, original);
			}
			...
			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |=
					layoutSEOSiteModelImpl.getColumnBitmask(attributeName);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						layoutSEOSiteModelImpl.getOriginalAttributeValue(
							attributeName);
				}
				else {
					arguments[i] = layoutSEOSiteModelImpl.getAttributeValue(
						attributeName);
				}
			}
