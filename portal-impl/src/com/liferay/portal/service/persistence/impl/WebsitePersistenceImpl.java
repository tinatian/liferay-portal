	...
	private static class WebsiteModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(websiteModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			WebsiteModelImpl websiteModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] = websiteModelImpl.getOriginalAttributeValue(
						attributeName);
				}
				else {
					arguments[i] = websiteModelImpl.getAttributeValue(
						attributeName);
				}
			}
