...
/**
 * The arguments resolver class for retrieving value from CPOptionCategory.
 *
 * @author Marco Leo
 * @generated
 */
@Component(
	immediate = true,
	service = {
		CPOptionCategoryModelArgumentsResolver.class, ArgumentsResolver.class
	}
)
public class CPOptionCategoryModelArgumentsResolver
	...
	public Object[] getArguments(
		...
		if (!checkColumn || (columnBitmask == 0)) {
			return _getValue(cpOptionCategoryModelImpl, columnNames, original);
		}
		...
		if (finderPathColumnBitmask == null) {
			...
			if (finderPath.isBaseModelResult() &&
				(CPOptionCategoryPersistenceImpl.
	...
	private static Object[] _getValue(
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			if (original) {
				arguments[i] = cpOptionCategoryModelImpl.getColumnOriginalValue(
					columnName);
			}
			else {
