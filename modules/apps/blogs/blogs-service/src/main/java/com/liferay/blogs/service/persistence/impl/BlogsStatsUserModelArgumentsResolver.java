...
/**
 * The arguments resolver class for retrieving value from BlogsStatsUser.
 *
 * @author Brian Wing Shun Chan
 * @generated
 */
@Component(immediate = true, service = ArgumentsResolver.class)
public class BlogsStatsUserModelArgumentsResolver implements ArgumentsResolver {
	...
	public Object[] getArguments(
		...
		if (!checkColumn || (columnBitmask == 0)) {
			return _getValue(blogsStatsUserModelImpl, columnNames, original);
		}
	...
	private static Object[] _getValue(
		BlogsStatsUserModelImpl blogsStatsUserModelImpl, String[] columnNames,
		boolean original) {
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			if (original) {
				arguments[i] = blogsStatsUserModelImpl.getColumnOriginalValue(
					columnName);
			}
			else {
	...
	static {
		...
		orderByColumnsBitmask |= BlogsStatsUserModelImpl.getColumnBitmask(
			"entryCount");
