	...
	private static class AccountModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(accountModelImpl, attributeNames, original);
			}

			long finderPathColumnBitmask = 0;

			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |= accountModelImpl.getColumnBitmask(
					attributeName);
			}
		...
		private Object[] _getValue(
			AccountModelImpl accountModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] = accountModelImpl.getOriginalAttributeValue(
						attributeName);
				}
				else {
					arguments[i] = accountModelImpl.getAttributeValue(
						attributeName);
				}
			}
