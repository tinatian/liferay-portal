...
public class UserTrackerPersistenceImpl
	...
	public void clearCache(UserTracker userTracker) {
		...
		EntityCacheUtil.removeResult(
			UserTrackerModelImpl.ENTITY_CACHE_ENABLED, UserTrackerImpl.class,
			userTracker, UserTrackerModelImpl.COLUMN_BITMASK_ENABLED);
	}
	...
	public void clearCache(List<UserTracker> userTrackers) {
		...
		for (UserTracker userTracker : userTrackers) {
			EntityCacheUtil.removeResult(
				UserTrackerModelImpl.ENTITY_CACHE_ENABLED,
				UserTrackerImpl.class, userTracker,
	...
	public UserTracker updateImpl(UserTracker userTracker) {
		...
		EntityCacheUtil.putResult(
			UserTrackerModelImpl.ENTITY_CACHE_ENABLED, UserTrackerImpl.class,
			userTracker, false, UserTrackerModelImpl.COLUMN_BITMASK_ENABLED);
	...
	public void afterPropertiesSet() {
		...
		_finderPathWithoutPaginationFindBySessionId = new FinderPath(
			...
			_ARGUMENTS_BIFUNCTION_MAP.get("SessionId"),
			_ORIGINAL_ARGUMENTS_BIFUNCTION_MAP.get("SessionId"));

		_serviceRegistrations.add(
			registry.registerService(
				FinderPath.class, _finderPathWithoutPaginationFindBySessionId,
				HashMapBuilder.<String, Object>put(
					"cache.name", FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION
				).build()));
		...
		_serviceRegistrations.add(
			registry.registerService(
				FinderPath.class, _finderPathCountBySessionId,
				HashMapBuilder.<String, Object>put(
					"cache.name", FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION
				).build()));
	}
	...
	static {
		_FINDER_PATH_BITMASK_MAP.put(
			"CompanyId", UserTrackerModelImpl.COMPANYID_COLUMN_BITMASK);

		_FINDER_PATH_BITMASK_MAP.put(
			"UserId", UserTrackerModelImpl.USERID_COLUMN_BITMASK);

		_FINDER_PATH_BITMASK_MAP.put(
			"SessionId", UserTrackerModelImpl.SESSIONID_COLUMN_BITMASK);
	}
	...
	static {
		_ARGUMENTS_BIFUNCTION_MAP.put(
			...
			(baseModel, checkColumns) -> {
				...
				if (!checkColumns ||
					((userTrackerModelImpl.getColumnBitmask() &
					  _FINDER_PATH_BITMASK_MAP.get("CompanyId")) != 0)) {

					return new Object[] {userTrackerModelImpl.getCompanyId()};
				}
		...
		_ARGUMENTS_BIFUNCTION_MAP.put(
			...
			(baseModel, checkColumns) -> {
				...
				if (!checkColumns ||
					...
					return new Object[] {userTrackerModelImpl.getUserId()};
				}
		...
		_ARGUMENTS_BIFUNCTION_MAP.put(
			...
			(baseModel, checkColumns) -> {
				...
				if (!checkColumns ||
					((userTrackerModelImpl.getColumnBitmask() &
					  _FINDER_PATH_BITMASK_MAP.get("SessionId")) != 0)) {

					return new Object[] {userTrackerModelImpl.getSessionId()};
				}
