...
public class AttachmentPersistenceImpl
	...
	public void activate(BundleContext bundleContext) {
		...
		_finderPathWithPaginationFindByMessageId = _createFinderPath(
			AttachmentImpl.class, FINDER_CLASS_NAME_LIST_WITH_PAGINATION,
			"findByMessageId", new String[] {"messageId"});
		...
		_finderPathCountByMessageId = _createFinderPath(
			...
			"countByMessageId", new String[] {"messageId"});
	}
	...
	private static class AttachmentModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(attachmentModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			AttachmentModelImpl attachmentModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						attachmentModelImpl.getOriginalAttributeValue(
							attributeName);
				}
				else {
					arguments[i] = attachmentModelImpl.getAttributeValue(
						attributeName);
				}
			}
