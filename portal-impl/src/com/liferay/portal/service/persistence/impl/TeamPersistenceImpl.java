	...
	public void afterPropertiesSet() {
		...
		_argumentsResolverServiceRegistration = registry.registerService(
			...
			HashMapBuilder.<String, Object>put(
				"model.class.name", Team.class.getName()
			).build());
	...
	private static class TeamModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(teamModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			TeamModelImpl teamModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] = teamModelImpl.getOriginalAttributeValue(
						attributeName);
				}
				else {
					arguments[i] = teamModelImpl.getAttributeValue(
						attributeName);
				}
			}
