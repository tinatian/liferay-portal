...
public class CTMessagePersistenceImpl
	...
	public void clearCache(List<CTMessage> ctMessages) {
		for (CTMessage ctMessage : ctMessages) {
			entityCache.removeResult(
				entityCacheEnabled, CTMessageImpl.class,
				ctMessage.getPrimaryKey(), ctMessage,
				new Object[] {
					_columnBitmaskEnabled,
					((CTMessageModelImpl)ctMessage).getColumnBitmask()
				});
	...
	public CTMessage updateImpl(CTMessage ctMessage) {
		...
		entityCache.putResult(
			entityCacheEnabled, CTMessageImpl.class,
			ctMessageModelImpl.getPrimaryKey(), ctMessageModelImpl, false,
			new Object[] {
				_columnBitmaskEnabled, ctMessageModelImpl.getColumnBitmask()
			});
	...
	public void activate(BundleContext bundleContext) {
		...
		Bundle bundle = FrameworkUtil.getBundle(CTMessagePersistenceImpl.class);
