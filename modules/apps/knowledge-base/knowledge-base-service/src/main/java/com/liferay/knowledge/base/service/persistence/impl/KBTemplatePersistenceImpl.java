	...
	private static class KBTemplateModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(kbTemplateModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			KBTemplateModelImpl kbTemplateModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						kbTemplateModelImpl.getOriginalAttributeValue(
							attributeName);
				}
				else {
					arguments[i] = kbTemplateModelImpl.getAttributeValue(
						attributeName);
				}
			}
