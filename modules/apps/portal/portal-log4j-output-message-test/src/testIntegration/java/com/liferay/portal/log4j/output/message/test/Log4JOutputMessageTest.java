...
public class Log4JOutputMessageTest {
	...
	private void _testFileOutput(
		...
		try (RandomAccessFile randomAccessLogFile = new RandomAccessFile(
				_textLogFile, "r");
			...
			String textFileName = _textLogFile.getName();

			Matcher matcher = _textFileNamePattern.matcher(textFileName);

			Assert.assertTrue(
				"Text log file name should match the pattern liferay." +
					"yyyy-MM-dd.log, but actual name is " + textFileName,
				matcher.matches());

			_assertTextLog(level, message, throwable, logOutputContent);

			// Assert xml file output

			String xmlFileName = _xmlLogFile.getName();

			matcher = _xmlFileNamePattern.matcher(xmlFileName);

			Assert.assertTrue(
				"XML log file name should match the pattern liferay." +
					"yyyy-MM-dd.xml, but actual name is " + xmlFileName,
				matcher.matches());

			_assertXmlLog(level, message, throwable, xmlOutputContent);
		}
		finally {
			_unsyncStringWriter.reset();
		}
