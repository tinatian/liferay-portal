	...
	public void activate(BundleContext bundleContext) {
		...
		_argumentsResolverServiceRegistration = _bundleContext.registerService(
			...
			MapUtil.singletonDictionary(
				"model.class.name", AuditEvent.class.getName()));

		_finderPathWithPaginationFindAll = _createFinderPath(
			AuditEventImpl.class, FINDER_CLASS_NAME_LIST_WITH_PAGINATION,
	...
	private static class AuditEventModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(auditEventModelImpl, attributeNames, original);
			}
		...
		private Object[] _getValue(
			AuditEventModelImpl auditEventModelImpl, String[] attributeNames,
			boolean original) {
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						auditEventModelImpl.getOriginalAttributeValue(
							attributeName);
				}
				else {
					arguments[i] = auditEventModelImpl.getAttributeValue(
						attributeName);
				}
			}
