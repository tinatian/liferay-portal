	...
	public void activate(BundleContext bundleContext) {
		...
		_argumentsResolverServiceRegistration = _bundleContext.registerService(
			...
			MapUtil.singletonDictionary(
				"model.class.name", KaleoProcess.class.getName()));
		...
		_finderPathFetchByDDLRecordSetId = _createFinderPath(
			...
			"fetchByDDLRecordSetId", new String[] {"DDLRecordSetId"});

		_finderPathCountByDDLRecordSetId = _createFinderPath(
			Long.class, FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION,
			"countByDDLRecordSetId", new String[] {"DDLRecordSetId"});
	}
	...
	private static class KaleoProcessModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			if (!checkColumn || (columnBitmask == 0)) {
				return _getValue(
					kaleoProcessModelImpl, attributeNames, original);
			}
			...
			for (String attributeName : attributeNames) {
				finderPathColumnBitmask |=
					kaleoProcessModelImpl.getColumnBitmask(attributeName);
			}
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					...
					arguments[i] = kaleoProcessModelImpl.getAttributeValue(
						attributeName);
				}
			}
